...
$(document).ready(function() {
     ...
     */
    ContrastProgress.initialise(contrastProgressBarSettings);
...
var BufferStatus = {
    ...
    createBufferDivs: function () {
        ...
        $('.timeline-chapters__state').each(function () {
...
var ContrastProgress = {
    ...
    initialise: function (contrastProgressBarSettings){
        ContrastProgress.SettingsJsonObject = contrastProgressBarSettings;

        if (ContrastProgress.SettingsJsonObject.isEnabled) {
    ...
    setInitialCssProperties: function () {
        ...
        // TODO see if these need to be re-enabled
    ...
    setIE7Compatibility: function () {
        if($('html').hasClass('ie7')){
            ...
            // TODO see if this needs to be re-enabled
...
var SocialIcons = {
    ...
    generateHtml: function() {
        ...
        $.each(SocialIcons.icons, function(key, icon) {
            html += '<button class="social__icon social__icon--' + key + '" title="' + icon.text + '" data-icon="' + key + '">';
...
var Timeline = {
    ...
    update: function() {
        var timeline_width = $('#jsTimelineContainer').width();
        $('#jsTimelineProgress, #jsTimelineIndicator').width(timeline_width * Timeline.getProgress());

        // TODO re-enable contrast timeline
    ...
    enableTimelineIfNecessary: function() {
        if ($(".timeline__cover") && VideoPlayerInterface.isSourceSet) {
            $(".timeline__cover").remove();
    ...
    disableTimelineIfNecessary: function() {
        $('<div class="timeline__cover"></div>').prependTo('.timeline');

        // TODO look at this
        // $('<div class="timeline-cover-playhead"></div>').prependTo('.timeline-wrapper');
    },
    ...
    updateSeenChapterColors: function(){
        ...
        $(".jsTimelineState").each(function (){
            ...
            if(percentStart < currentProgress ){
                $(this).siblings('.timeline-chapters__label').css('color', Timeline.SettingsJsonObject.SeenChaptersFontColor);
            }
        });
    },
...
var iFrameEvents = {
    ...
    closeLowBandwidthCard: function(element) {
        ...
        if ($lowBandwidthBar.length > 0) {
            $lowBandwidthBar.appendTo(".player-column");
        }
